...
def annealed_vargrad(
    log_prior: torch.Tensor,
    log_pfs: torch.Tensor,
    log_pbs: torch.Tensor,
    log_reward_t: torch.Tensor,
):
    batch_mean = (log_pfs + log_reward_t[:, :-1] - log_pbs - log_reward_t[:, 1:]).mean(
        dim=0
    )

    loss = 0.5 * (
        (log_pfs + log_reward_t[:, :-1] - log_pbs - log_reward_t[:, 1:] - batch_mean)
        ** 2
    ).sum(-1)
...
def vargrad_loss(
    ...
    log_Z = (log_reward + log_pbs.sum(-1) - log_pfs.sum(-1) - log_prior).mean(
        dim=0, keepdim=True
    )
