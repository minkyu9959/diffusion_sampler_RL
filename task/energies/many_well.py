...
class ManyWell(BaseEnergy):
    ...
    def log_prob(self, x: torch.Tensor):
        ...
        x = x.reshape(-1, 2)
        logit = self.doublewell_log_prob(x).view(*batch_size, self.n_wells).sum(dim=-1)
...
class SymmetricManyWell(BaseEnergy):
    ...
    def __init__(self, device: str, dim: int = 32):
        ...
        # as rejection sampling proposal
        self.component_mix = torch.tensor([0.5, 0.5])
        self.means = torch.tensor([-1.7, 1.7])
        self.scales = torch.tensor([0.5, 0.5])
